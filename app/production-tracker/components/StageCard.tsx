...
import {
  ...
  StageState,
  StageDeviceInfo,
} from "@/app/production-tracker/types";
import {
  Play,
  Pause,
  AlertTriangle,
...
export default function StageCard({
  ...
  const statusInfo = STATUS_MAP[resolvedStatus] ?? {
  ...
  const realtimeArea =
    product?.specs?.width && product?.specs?.height && output > 0
      ? parseFloat(
          (
            ((product.specs.width * product.specs.height) / 1000000) *
            output
          ).toFixed(2)
        )
      : 0;
  ...
  const highestPositionDevices =
    devices?.filter((d) => d.position === maxPosition) ?? [];
  ...
  const stageTotalArea =
    product?.specs?.width && product?.specs?.height && stageTotalQuantity > 0
      ? parseFloat(
          (
            ((product.specs.width * product.specs.height) / 1000000) *
            stageTotalQuantity
    ...
    >
      <div className={styles.stageCardHeader}>
        ...
        <div className={styles.stageHeaderActions}>
          <span className={`${styles.stageStatusChip} ${statusInfo.chipClass}`}>
            {statusInfo.label}
          </span>
        ...
        <div className={styles.stageRealtimeMetric}>
          ...
          {devices && devices.length > 0 ? (
            <div className={styles.deviceQuantitiesList}>
              {devices.map((device) => {
                const deviceQty =
                  state.deviceQuantities?.[device.deviceId] ?? 0;
                const formattedDeviceQty = formatInteger(deviceQty);
                return (
                  <div
                    key={device.deviceId}
                    className={styles.deviceQuantityRow}
                  >
                    <span className={styles.deviceName}>{device.name}</span>
                    <div className={styles.deviceQuantityValueContainer}>
                      <span className={styles.deviceQuantityValue}>
                        {formattedDeviceQty}
                      </span>
              ...
              {devices.map((device) => {
                ...
                const deviceArea =
                  ...
                  deviceQty > 0
                    ? parseFloat(
                        (
                          ((product.specs.width * product.specs.height) /
                            1000000) *
                          deviceQty
                        ).toFixed(2)
                      )
        ...
        <div className={styles.stageTotalMetric}>
          ...
          <div className={styles.stageTotalRow}>
            <div className={styles.stageTotalItem}>
              <span className={styles.stageTotalValue}>
                {formattedStageTotalQty}
              </span>
